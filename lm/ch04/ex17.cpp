...
{
    const auto props =
        MMM::Properties<std::string>({"bbb", "zzz", "aaa", "frg"});
...
{
    const auto props =
        MMM::Properties<std::string>({"bbb", "zzz", "aaa", "bbb", "frg"});
    ASSERT_EQ(std::set<std::string>{"bbb"}, props.getMode());
}

TEST(LMCh04Ex17, willReturnTwoModes)
{
    const auto props = MMM::Properties<std::string>(
        {"bbb", "zzz", "aaa", "bbb", "frg", "zzz"});
