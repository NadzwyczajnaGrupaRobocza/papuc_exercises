...
Calculator::Calculator(const Data& data)
    : first(data.first), second(data.second), operation(data.operation),
      result(0.0)
{
}
...
bool Calculator::operationIsNotAcceptable(const std::string& aOperation)
{
    ...
    if (aOperation != "+" and aOperation != "-" and aOperation != "*" and
        aOperation != "/")
    {
...
double Calculator::calculate()
{
    ...
    switch (cOperation)
    {
    case '+': operationResult = first + second; break;
    case '-': operationResult = first - second; break;
    case '*': operationResult = first * second; break;
    case '/': operationResult = first / second; break;
    default:
...
std::string Calculator::printResult()
{
    ...
    {
    case '+':
        output << "Sum of " << first << " and " << second << " is equal to "
               << result;
        break;
    case '-':
        ...
        break;
    case '*':
        output << "Product of " << first << " and " << second << " is equal to "
               << result;
        break;
    case '/':
